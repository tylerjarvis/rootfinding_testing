...
for dim in dims:
    for v, arg in enumerate(args):
        YR = open("yroots_results/nov1/dim{}_{}_randn.txt".format(dim, arg))
        YR = stacktoarray(YR.read())

        JR = open("jroots_results/nov1/dim{}_{}.txt".format(dim, arg))
        JR = stacktoarray(JR.read())
        # JR = JR[:9]

        TR = open("telen_results/dim{}_{}.txt".format(dim, arg))
        TR = stacktoarray(TR.read())

        if arg == "avg_times":
            BR = np.load("bertini_results/dim{}_{}.npy".format(dim, arg))
        else:
            BR = open("bertini_results/dim{}_{}.txt".format(dim, arg))
            BR = stacktoarray(BR.read())

        if dim in [2, 3]:
            NR = open("NSolveValues_results/dim{}_{}2_cont.txt".format(dim, arg))
        else:
            NR = open("NSolveValues_results/dim{}_{}_cont.txt".format(dim, arg))
        NR = stacktoarray(NR.read())
        if False and arg != "avg_times":
            NRM = np.load("NSolveValues_results/dim2_missing_roots.npy")
            NRD = np.load("NSolveValues_results/dim2_double_roots.npy")

        RR = open("reduce_results/dim{}_{}.txt".format(dim, arg))
        RR = stacktoarray(RR.read())

        if dim == 2:
            CR = open("chebfun_results/dim{}_{}.txt".format(dim, arg))
