...
function poly_from_terms(coeff_data, dim::Int, test_num, func_num)
    exps   = Int.(coeff_data[test_num, func_num, :, 1:dim])
    coeffs = coeff_data[test_num, func_num, :, dim + 1]

    return function (x...)
        @assert length(x) == dim
        s = 0.0
        @inbounds for k in axes(exps, 1)
            m = 1.0
            for j in 1:dim
                ej = exps[k, j]
                if ej != 0
...
for deg in degs
    ...
    flush(stdout)
    for i in 1:10
        f = [poly_from_terms(coeffs, dim, i, j) for j in 1:dim];
